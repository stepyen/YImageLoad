...
public class WebpDrawable extends Drawable implements WebpFrameLoader.FrameCallback, Animatable {
    ...
    public ImageLoopStatusCallBack getLoopStatusCallBack() {
        return loopStatusCallBack;
    }

    public void setLoopStatusCallBack(ImageLoopStatusCallBack loopStatusCallBack) {
        this.loopStatusCallBack = loopStatusCallBack;
    }
    ...
    public void onFrameReady() {
        if(this.getCallback() == null) {
            ...
            this.invalidateSelf();
            if(this.getMaxLoopCount() != LOOP_FOREVER) {
                boolean loopFinish = this.loopCount >= this.getMaxLoopCount();
                if(loopFinish) {
                    this.stop();
                }else {
                    if (getLoopStatusCallBack() != null) {
                        int currentLoopCount = loopFinish?getMaxLoopCount():(loopCount +1);
                        int frameCount = getFrameCount();
                        getLoopStatusCallBack().onLoopCallback(loopFinish,
                                currentLoopCount, frameCount, getFrameIndex() + 1);
                    }
                }
            }
    ...
    public void setLoopCount(int loopCount) {
        if(loopCount <= 0 && loopCount != LOOP_FOREVER && loopCount != LOOP_INTRINSIC) {
            ...
            if(loopCount == LOOP_INTRINSIC) {
                ...
                setMaxLoopCount(intrinsicCount == LOOP_INTRINSIC ? LOOP_FOREVER : intrinsicCount);
            } else {
                setMaxLoopCount(loopCount);
            }
